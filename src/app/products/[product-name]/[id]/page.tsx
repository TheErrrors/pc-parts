              ...
              <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
                {Object.entries(productData.specs).map(([key, value]) => (
                  ...
                  >
                    ...
                    <span className='font-body-sm font-semibold text-on-surface'>{value}</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                {onlineStores.map((store, index) => (
                  ...
                  >
                    <div className='flex flex-col gap-1.5 flex-1'>
                      <div
                        className={`font-body-md font-semibold ${store.inStock ? 'text-on-surface' : 'text-on-surface-variant'}`}
                      >
                        {store.name}
                      </div>
                    ...
                    <div className='flex items-center justify-between sm:justify-end gap-6 flex-1 sm:flex-none'>
                      <div
                        className={`font-price-lg tabular-nums ${store.inStock ? 'text-primary' : 'text-on-surface-variant/70'}`}
                      >
                        ₹{store.price.toLocaleString('en-IN')}
                      </div>
                      <Button
                        variant={store.inStock ? 'primary' : 'outline'}
                        className={`shrink-0 ${!store.inStock && 'text-on-surface-variant border-surface-container-high hover:bg-surface-container'}`}
                      >
                    ...
                    >
                      <div className='flex items-start gap-2'>
                        ...
                        <div className='flex flex-col'>
                          ...
                          <span
                            className={`font-label-md text-[11px] mt-1 ${location.trend < 0 ? 'text-[#22c55e]' : 'text-red-500'}`}
                          >
          ...
          <div className='bg-surface-container-lowest border border-surface-container-high rounded-2xl p-6 mb-12'>
            ...
            <div className='h-[300px] w-full bg-surface rounded-xl border border-surface-container-high relative overflow-hidden flex flex-col justify-between py-6'>
              ...
              <div className='absolute inset-0 flex items-center justify-center pointer-events-none'>
                <span className='text-on-surface-variant/50 font-body-sm font-medium'>
